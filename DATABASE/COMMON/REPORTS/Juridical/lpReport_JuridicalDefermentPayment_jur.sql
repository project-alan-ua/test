...
DROP FUNCTION IF EXISTS lpReport_JuridicalDefermentPayment_jur (TDateTime, TDateTime, TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Integer, Integer);

CREATE OR REPLACE FUNCTION lpReport_JuridicalDefermentPayment_jur(
...
BEGIN
     ...
     -- Результат
     RETURN QUERY
     WITH tmpReport AS (SELECT 0 AS BranchId, 0 AS InfoMoneyId, 0 AS Sale_Summ
                       )
                ...
                (SELECT Container.Id           AS ContainerId
                  ...
                  WHERE CLO_Juridical.DescId = zc_ContainerLinkObject_Juridical()
                   ...
                   --AND MIContainer.MovementDescId = zc_Movement_Income()
                     ...
                     AND (View_Contract_ContractKey.ContractId_Key = vbContractId_key OR vbContractId_key = 0)
...
-- тест
-- SELECT * FROM lpReport_JuridicalDefermentPayment_jur (inOperDate:= CURRENT_DATE, inEmptyParam:= NULL :: TDateTime, inStartDate_sale:= CURRENT_DATE, inEndDate_sale:= CURRENT_DATE, inAccountId:= 0, inPaidKindId:= zc_Enum_PaidKind_SecondForm(), inBranchId:= 0, inJuridicalGroupId:= null, inJuridicalId:=0, inContractId:= 0, inUserId:= 5);
