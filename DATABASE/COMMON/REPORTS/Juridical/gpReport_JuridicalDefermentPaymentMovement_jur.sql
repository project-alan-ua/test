-- Function: gpReport_JuridicalDefermentPaymentMovement_jur()

DROP FUNCTION IF EXISTS gpReport_JuridicalDefermentPaymentMovement_jur (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_JuridicalDefermentPaymentMovement_jur(
...
BEGIN
     ...
     ELSE
         ...
         CREATE TEMP TABLE _tmpReport ON COMMIT DROP AS
                      (SELECT tmpReport.*
                       FROM lpReport_JuridicalDefermentPayment_jur (inOperDate         := inOperDate
...
-- тест
-- SELECT * FROM gpReport_JuridicalDefermentPaymentMovement_jur(inOperDate := ('04.07.2024')::TDateTime , inEmptyParam := ('01.01.2016')::TDateTime , inAccountId := 9128 , inPaidKindId := 3 , inBranchId := 0 , inJuridicalGroupId := 0 , inJuridicalId:=0, inContractId:= 0, inSession := '378f6845-ef70-4e5b-aeb9-45d91bd5e82e') --WHERE SaleSumm <> 0
